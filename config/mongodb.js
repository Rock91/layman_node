...
async function find({
    ...
    return await getModel(db, model).find(query, project, options).populate(populate).limit(limit).skip(skip).sort(sort).lean().exec();
}
...
async function findOne({
    ...
    return await getModel(db, model).findOne(query, project).sort(sort).lean().exec();
}
...
async function aggregate2({
    ...
    return await getModel(db, model).aggregate(pipeline).populate(populate).exec();
}
...
async function findByIdAndUpdate({
    ...
    return await getModel(db, model).findByIdAndUpdate({
        _id: ObjectId(query)
    }, update, options).lean();
}
...
async function findOneAndUpdate({
    ...
    return await getModel(db, model).findOneAndUpdate(query, update, options).lean();
}
...
let ggDb = new DB(config.mongoose.master_db);
