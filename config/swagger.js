...
const { getFullPath, getFileExtension, getAllFilesFromFolder } = require('../utils/file');
const config = require('./config');
const swaggerConfig = (appOrRouter, { basedir, filedir, files, url }) => {
    // return;
    const fileList = files ? files : getAllFilesFromFolder(getFullPath(basedir, filedir)).filter(e => getFileExtension(e) == '.js');
