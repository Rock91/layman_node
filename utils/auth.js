...
function sign(payload, option) {
    return new Promise(resolve => {
        ...
        if(payload.gameId){
            sercurData.gameId=payload.gameId;
        }
...
async function authMiddleware(req, res, next) {
    ...
    let authToken = req.headers['authorization'] ? req.headers['authorization'].replace('Bearer ', '') : null;
    ...
    if(error || !decoded || !decoded.exp || new Date(decoded.exp * 1000).getTime() < new Date().getTime()) {
...
const resetPasswordVerify = async(payload)=>{
    ...
    console.log("resetPasswordVerify : time condition : ",new Date(tokenDoc.expires).getTime(), new Date().getTime());
    if(new Date(tokenDoc.expires).getTime() < new Date().getTime()){
...
async function decodeHeader(req)
{
    ...
    if(decoded && decoded.is_Block){
        throw new ApiError(httpStatus.BAD_REQUEST, "You are blocked by superAdmin!")  
    }
    if(decoded) 
    { 
        return req.user = {
...
async function passwordValidation(password){ 
    ...
    if(password.length >= 8){
        ...
        if(!( /[`!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/.test(password)))
            return {flag:true,msg:"At least one special character (eg. &@*#!) required"}

        return {flag:false,msg:""}
    }else{
        return {flag:true,msg:"Password length must be greater than or equal to 8"}
    }   
}
module.exports = {
    generateToken,
